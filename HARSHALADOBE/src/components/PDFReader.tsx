...
export function PDFReader({ documents, persona, jobToBeDone, onBack }: PDFReaderProps) {
  ...
  const [activeRightPanel, setActiveRightPanel] = useState<'insights' | 'strategic' | 'connections' | 'podcast' | 'accessibility' | 'simplifier' | 'export' | 'highlights' | 'analytics' | 'bookmarks' | null>(null);
  ...
  const handleTextSelection = async (text: string, page: number) => {
    ...
    if (insightMode) {
      ...
      // Show toast about selected text being analyzed
      toast({
        title: "Text Selected for Analysis",
        description: `"${text.substring(0, 50)}${text.length > 50 ? '...' : ''}" - Analyzing across all documents`,
      });
      
      // Generate real-time cross-document insights
      if (text.length > 20 && persona && jobToBeDone) {
        try {
          // Generate insights for the selected text
          await generateInsightsForText(text);
          
          // Show success message
          toast({
            title: "Real-time Insights Generated",
            description: `Analyzing text across all documents for related, overlapping, and contradicting information.`,
          });
        } catch (error) {
          console.error('Failed to generate real-time insights:', error);
          toast({
            title: "Insights Generation Failed",
            description: "Unable to generate cross-document insights for selected text.",
            variant: "destructive"
          });
        }
      }
    ...
    // Automatically generate insights for selected text if conditions are met (non-insight mode)
    if (!insightMode && text.length > 50 && persona && jobToBeDone) {
      try {
        await generateInsightsForText(text);
    ...
    } else if (!insightMode && text.length > 50) {
      toast({
        title: "Text Selected",
    ...
    currentDocument.outline.forEach((item, index) => {
      if (index < 8) { // Limit to first 8 sections
        const highlight: Highlight = {
          ...
          explanation: `Important section for ${persona || 'your role'}: ${item.title.slice(0, 100)}...`
        };
  ...
  return (
    <div className="h-screen flex flex-col bg-background">
      ...
      {documents && documents.length > 0 && (
        <div className="border-b border-border-subtle bg-surface-elevated/80 px-4">
          <div className="overflow-x-auto no-scrollbar">
            <Tabs
              value={currentDocument?.id || (documents[0]?.id ?? '')}
              onValueChange={(val) => {
                const nextDoc = documents.find(d => d.id === val);
                if (nextDoc) {
            ...
            >
              <TabsList className="h-11 bg-transparent p-1">
                {documents.map((doc) => (
                  <TabsTrigger
                    key={doc.id}
                    value={doc.id}
                    className="data-[state=active]:bg-background/90 data-[state=active]:text-foreground data-[state=active]:shadow-sm mr-1 rounded-md px-3 py-2 text-sm max-w-[18rem] truncate"
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {/* Right Panel - Full Dedicated Mode */}
        {rightPanelOpen && (
          <aside className="w-96 max-w-96 min-w-0 bg-surface-elevated/50 border-l border-border-subtle flex flex-col animate-fade-in backdrop-blur-sm overflow-hidden">
            {/* Show Mode Selection Only When No Mode is Active */}
            {!activeRightPanel && (
              <div className="p-4 border-b border-border-subtle min-w-0">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-foreground">Select a Tool</h3>
                  <Button
                    variant="ghost"
                    size="sm"
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      onClick={() => setActiveRightPanel(key as any)}
                      className="gap-2 h-10 justify-start text-xs hover:bg-surface-hover"
                    >
            ...
            {/* Show Mode Header and Close Button When Mode is Active */}
            {activeRightPanel && (
              <div className="p-4 border-b border-border-subtle min-w-0 flex items-center justify-between">
                <h3 className="text-sm font-semibold text-foreground">
                  {activeRightPanel === 'insights' && 'Real-time Insights'}
                  {activeRightPanel === 'strategic' && 'Strategic Analysis'}
                  {activeRightPanel === 'connections' && 'Cross-Document Connections'}
                  {activeRightPanel === 'podcast' && 'Podcast Generator'}
                  {activeRightPanel === 'accessibility' && 'Accessibility Tools'}
                  {activeRightPanel === 'simplifier' && 'Text Simplifier'}
                  {activeRightPanel === 'export' && 'Export & Share'}
                  {activeRightPanel === 'highlights' && 'Document Highlights'}
                  {activeRightPanel === 'analytics' && 'Reading Analytics'}
                  {activeRightPanel === 'bookmarks' && 'Smart Bookmarks'}
                </h3>
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    className="h-8 px-2 text-xs hover:bg-surface-hover"
                  >
                    Back to Tools
                  </Button>
                  <Button
